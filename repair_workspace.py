...
def main():
    print("[+] Fixing static routing in main.py...")
...
"""
    ...
    print("  [?] Fixed main.py static mounting paths.")

    # Commit fix to git
    print("[+] Committing fix to GitHub...")
    subprocess.run(["git", "add", "main.py"], check=True)
    subprocess.run(["git", "commit", "-m", "fix(routing): mount static files properly to resolve 404 on assets"], check=True)
